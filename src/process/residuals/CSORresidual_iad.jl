...
"""
function CSOR_residual_iad!(resid, state_variables, sensitivity_parameters, constants)
...
function compute_CSOR_residual!(
    ...
    @. solve_containers.deltas = solve_containers.sigr_est - sigr

    sigr .= solve_containers.sigr_est

    update_CSOR_residual_values_iad!(
...
- `pf2::Float=0.5` : press forward factor 2
"""
function relax_gamw!(gamw, delta_prev, delta; nrf=0.4, btw=0.6, pfw=1.2, test=false)
...
"""
function update_CSOR_residual_values_iad!(
...
"""
function check_CSOR_convergence_iad!(
    conv,
    resid,
    state_dims;
    f_circ=1e-3,
    ...
    conv[] =
        maximum(resid[state_dims.Gamr.index]) < f_circ &&
        maximum(resid[state_dims.gamw.index]) < f_dgamw
