function visualize_paneling(
    ...
    if control_points
        plot!(
            panels.control_point[:, 1],
            panels.control_point[:, 2];
            color=red[2],
            seriestype=:scatter,
            markershape=:rect,
    ...
    if normals
        for i in 1:length(panels.length)
            lab = i == 1 ? "Normals" : ""
            plot!(
                [0.0; normal_scaling * panels.normal[i, 1]] .+ panels.control_point[i, 1],
                [0.0; normal_scaling * panels.normal[i, 2]] .+ panels.control_point[i, 2];
                label=lab,
                color=blue[3],
            )
        end
