...
function rotorzloc_lift(
    clmax=2.0, clmin=-1.5, alpha0=0.0, dclda=2.0 * pi, dclda_stall=-0.1, blend_hardness=5
)
    function fmax(cl)
        return maximum(
            smooth_cl(cl, clmin, alpha0, dclda, dclda_stall, blend_hardness)[2]
        ) - clmax
    end

    function fmin(cl)
        return minimum(
            smooth_cl(clmax, cl, alpha0, dclda, dclda_stall, blend_hardness)[2]
        ) - clmin
    end

    trueclmax = Roots.find_zero(fmax, (0.0, clmax))
    trueclmin = Roots.find_zero(fmin, (0.0, clmin))

    return smooth_cl(trueclmax, trueclmin, alpha0, dclda, dclda_stall, blend_hardness)
end
...
function smooth_cl(
    ...
    cl_alpha = dclda * (alpha .- alpha0)

    # invert cl function to be able to find alphas for clmax and min
    fa(cl) = cl / dclda + alpha0

    #angles of attack at clmax and clmin
    #these are the transition locations for the sigmoid blending
    amin = fa(clmin)
    amax = fa(clmax)
...
"""
function rotorzloc_drag(cl, Re, cdo=5e-3, clo=0.15, b=4e-3, Re_ref=5e5, f=nothing)
    if isnothing(f)
        #=
        Typically:
              f = -0.1 to -0.2  for high-Re turbulent flow Re > 2M
...
function generate_airfoil_data(lift_params, drag_params, Re; filename=nothing)
    ...
    alpha, cl = rotorzloc_lift(clmax, clmin, alpha0, dclda, dclda_stall, blend_hardness)

    cd = rotorzloc_drag(cl, Re, cdo, clo, b, Re_ref, f)
