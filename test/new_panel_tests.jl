@testset "Panel Geometry" begin
    ...
    @test all(isapprox.(panels.tencrossn[2, :], [sqrt(2) / 2, -1.0]))
    ...
    @test panels.tendotn[1, 1] ==
        dt.dot(panels.tenormal[1, :], panels.normal[panels.endpanelidxs[1, 2], :])
    @test panels.tendotn[1, 2] ==
        dt.dot(panels.tenormal[1, :], panels.normal[panels.endpanelidxs[1, 1], :])
    @test panels.tendotn[2, 1] ==
