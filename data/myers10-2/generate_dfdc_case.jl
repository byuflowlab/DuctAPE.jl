...
function generate_input_file(;
    ...
    # Rotor Section Data
    xdisk=0.35, #axial location of rotor #TODO what are the units and datum?
    ...
    rblade=[], #radial station locations in meters
    chord=[], #chord lengths at radial stations in meters
    beta=[], #local twist angle in degrees at radial stations
    # Drag Section Data
    pts=0, #number of points defining drag object
    xdrag=[], #x cooridnates of points #TODO units and datum?
    rdrag=[], #r coordinates of points #TODO units and datum?
    ...
    if pts > 0
        dragsection!(f; pts=pts, x=xdrag, r=rdrag, cda=cda, tab=tab)
    end

    geometrysection!(f; case_name=case_name, x=xduct, y=yduct, tab=tab)
...
function aerosection!(
    ...
    for i in 1:nsections
        ...
        write(f, "$tab$(reref[i])$tab$tab$(reexp[i])\n")
    end
