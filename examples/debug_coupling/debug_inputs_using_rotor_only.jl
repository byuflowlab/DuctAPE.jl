#for debugging, get working rotor-only inputs for wake
_, roinputs = dt.initialize_rotor_states(rotor_parameters, freestream; wake_length=1.0)


# number of rotors
nrotor = roinputs.num_rotors
...
xgrid = roinputs.x_grid_points
rgrid = roinputs.r_grid_points
wake_vortex_panels = roinputs.wake_vortex_panels

#------------------------------------------#
# Generate Rotor Panels and Blade Elements #
#------------------------------------------#

rotor_source_panels = roinputs.rotor_source_panels
...
@assert all( vx_rw .== roinputs.vx_rw)
@assert all( vx_rr .== roinputs.vx_rr)
...
inputs = (;
    ...
    # panels
    rotor_indices,
    num_wake_x_panels=length(roinputs.xrange) - 1,
    num_body_panels,
