...
##### ----- Visualize to Check ----- #####
visualize_paneling(panels; coordinates=duct_coordinates, control_points=true, nodes=true, normals=true, savepath=savepath, filename="body-geometry.pdf")
...
LHSlsq_nokutta, RHSlsq_nokutta = dt.prep_leastsquares(LHSnokutta, RHSnokutta, prescribedpanels)
...
Vb_nokutta_wake = dt.vfromTE(panels.control_point, panels.TEnodes, panels.TEidxs, mu_nokutta)
...
plot!(pp, pressurexupper, pressureupper, seriestype=:scatter, color=blue[1], markershape=:utriangle, label="exp outer")
plot!(pp, pressurexlower, pressurelower, seriestype=:scatter, color=blue[1], markershape=:dtriangle, label="exp inner")
...
plot!(pv, Vs_over_Vinf_x, Vs_over_Vinf_vs, seriestype=:scatter, color=blue[1], markershape=:utriangle, label="experimental")
xs = panels.control_point[:,1]
