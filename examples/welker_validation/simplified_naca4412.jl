...
include(project_dir * "/examples/simple_airfoil.jl")
...
generate_airfoil_data(
    lift_params, drag_params, Re; filename=project_dir * "examples/simple_naca4412.dat"
)
...
ls = []
ds = []
open(project_dir * "/examples/simple_naca4412.dat") do f
    for line in eachline(f)
        parts = split(line)
