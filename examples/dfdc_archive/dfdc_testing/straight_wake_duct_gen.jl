...
include(project_dir * "/examples/dfdc_testing/generate_dfdc_case.jl")

using FLOWFoil

function gen_straightwake_case(;savepath="examples/dfdc_testing/")
...
function plot_geom(duct_coordinates, hub_coordinates, xrotor, r, c, t)
    ...
    savefig("examples/dfdc_testing/straightwake-geom.pdf")

    plot(; xlabel="r/Rtip", ylabel="chord (m)")
    plot!(r / r[end], c)
    savefig("examples/dfdc_testing/straightwake-chord.pdf")

    plot(; xlabel="r/Rtip", ylabel="twist (deg)")
    plot!(r / r[end], t)
    savefig("examples/dfdc_testing/straightwake-twist.pdf")
